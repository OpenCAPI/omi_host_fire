...
# Test to calibrate the DL2DL interface and send C3S commands in a loop
...
write_reg(0x20100, 0x80003000); # Valid, GOTO 0, repeat 3 times (4 total)
# No END
...
# Stop C3S in Fire 0
# OR in stop bit to not overwrite other config bits
write_reg(0x30000, 0x2 | read_reg(0x30000));

$cycle_start = 0;
fusion::currentCycleL($cycle_start);
my $cycle = $cycle_start;
while (($cycle - $cycle_start) < 1000) {
    fusion::currentCycleL($cycle);
